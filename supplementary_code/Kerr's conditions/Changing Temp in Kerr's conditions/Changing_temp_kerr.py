...
def get_rotational_spectrum(T, ground_B, delta_B, delta_C, zeta, sigma, conditions = None):
   ...
   # np.savetxt('condition_c_smooth.txt',  smooth_data, delimiter= " ")
    ...
    #plt.plot(x - x[minima_ind][-2], y + 0.02, label = 'Simulated at ' + str(T) + ' K') #, '+ str(peak_sep_qr - peak_sep_pq))
    plt.plot((x + 0.04 + condition), y + 0.02 , label = 'Simulated at ' + str(T) + ' K') #, '+ str(peak_sep_qr - peak_sep_pq))
    ...
    plt.title('B = ' + str(ground_B) + ' cm$^{-1}$ 'r'  $\Delta B =$ ' + str(delta_B) + '% ,  'r'$\Delta C =$ ' + str(delta_C) + '%' , fontsize = 15)
...
plt.plot(((1/HD166937['Wavelength'])- 0.0001512079)*1e8, 1-(1-HD166937['Flux'])/(1-0.903)*0.1 , color = 'red', label = 'HD 166937')
plt.xlim(-2.5, 2.5)
...
plt.plot((((1/HD185418['Wavelength'])- 0.0001512079)*1e8)-0.1, 1-(1-HD185418['Flux'])/(1-0.822)*0.1, color = 'green', label = 'HD 185418')
