...
for on in object_names:
    ...
    #Fit_data = pd.read_csv(r"/Users/charmibhatt/Desktop/Local_GitHub/edibles/edibles/utils/simulations/Charmi/fitting methods/results_166937_triple_peak_-1_to_2.csv")
    ...
    # Obs_data_trp = Obs_data [(Obs_data['Flux'] <= 0.96)] #trp = triple peak structure
    ...
    def get_rotational_spectrum(xx, B, T, delta_B, zeta, sigma, origin):
       ...
       # plt.subplot(2,1,1)
        ...
        plt.plot(Obs_data_trp['Wavelength'], Obs_data_trp['Flux'], linewidth = 3, label = 'Data')
        #plt.show()

        plt.plot(model_data[:,0], model_data[:,1], linewidth = 2.8, label = 'Model')
        #plt.plot(simu_wavelength, simu_intenisty, color = 'red', label = on)
        #plt.xlim(-7.5, 7.5)
