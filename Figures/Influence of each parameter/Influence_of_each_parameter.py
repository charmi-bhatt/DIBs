...
def get_rotational_spectrum(T, ground_B, delta_B, delta_C, zeta, sigma):
    ...
    #axes[m].xticks(fontsize=14)
    #axes[m].set_xticklabels(fontsize = 15)
    for tick in axes[m].xaxis.get_major_ticks():
        tick.label.set_fontsize(14)
    for tick in axes[m].yaxis.get_major_ticks():
        tick.label.set_fontsize(14)

    
    axes[m].legend(loc='lower right', fontsize= 11)
    axes[m].set_ylabel('Intensity', rotation=90, labelpad=7, fontsize = 15)
    axes[4].set_xlabel('Wavenumber (cm$^{-1}$)', labelpad =10, fontsize = 15)
